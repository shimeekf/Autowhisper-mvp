...
function computeOilReminder(profile) {
  ...
  const daysSince = profile.lastOilChangeDate ? Math.floor((Date.now() - new Date(profile.lastOilChangeDate).getTime()) / (1000 * 60 * 60 * 24)) : null;
  ...
  return { due: Boolean(dueByDays || dueByMiles), reason };
}
